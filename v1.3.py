...
def correctCoordinate():
    Gravity = -9.80665
    anvNacc = read() #data input
    aver_sample_num = 30000 #1000 sample as 1 sec.
    initialAccelation = np.asarray([0, 0, Gravity], dtype=np.float64).reshape(3, 1)
...
def main():
    ...
    vel_xyz = np.zeros(3,dtype=np.float64)
    ...
    '''
    plt.figure()
    plt.title('V_Trajectory')
    plt.plot(x_record,color='green',label='x_rec')
    plt.plot(y_record,color='red',label='y_rec')
    plt.plot(z_record,color='blue',label='z_rec')
